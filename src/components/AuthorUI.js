...
import { commonstyles, blackcolor, whitecolor, bluecolor } from '../styles/commonstyles';
...
function AuthorUI({
  ...
  const renderLoadMoreButton = () => {
    if (loadingMore) {
      return <ActivityIndicator style={{ marginVertical: 16 }} size="small" color={blackcolor} />;
    }
  ...
  const renderItem = ({ item }) => (item.type === 'one' ? renderItemOne({ item }) : renderItemTwo({ item }));
      ...
      {!isTopNavigation && (
        <View style={HeaderStyle.DetailsHeader}>
          <TouchableOpacity onPress={() => navigation.goBack()}>
            <Image source={require('../Assets/Images/arrow.png')} style={{ width: 22, height: 22 }} />
          </TouchableOpacity>
        </View>
      )}
