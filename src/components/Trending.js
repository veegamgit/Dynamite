...
import {blackcolor, bluecolor, light_gray, whitecolor} from '../styles/commonstyles';
...
const Trending = React.memo(() => {
  ...
  const trendRederItem = ({item, index}) => {
    return (
        <Ripple style={{flexDirection: 'row', alignItems: 'center'}} key={index} onPress={()=> navigation.push('Topics', {
