...
import {Image, Share, Text, View, StyleSheet, Dimensions} from 'react-native';
...
const ShortsComponent = props => {
  const {item, index, propsdata, navigation} = props;
    ...
    <View style={styles.wrapper}>
      <View>
        ...
        <View
          ...
          <HandlePressable onPress={sharecall} style={commonstyles.iconPress}>
            <Image
      ...
      {index < 3 && (
        <View style={styles.swipeupWrapper}>
          ...
          />
          <Text style={styles.swipeUpText}>{t('swipeupfornextshorts')}</Text>
        </View>
      )}
      {index + 1 === propsdata.length && (
        <Text style={styles.noMoreSwipes}>{t('nomoreshortstoswipe')}</Text>
      )}
    </View>
  );
};
